...
using LangGraph, powered by Google Gemini Flash (free tier, auto-selects a live model).
...
# ======================================================================
# LLM ACCESS (Gemini Flash, free tier via Google AI Studio)
#
# Google has been renaming/retiring "Flash" model IDs at a fast clip
# (1.5 -> 2.0 -> 2.5 -> 3 -> 3.1 -> 3.5, several retired early/without
# notice). Rather than hardcode one string that goes stale again, we try
# Google's own "-latest" alias first (it always points at their current
# recommended Flash model), then fall through a short list of known
# generations, and cache whichever model name actually answers so we
# don't re-pay the failed attempts on every call.
# ======================================================================
...
def to_ascii(text: str) -> str:
    """The Google client's request pipeline has been observed to raise
    'ascii codec can't encode character' on plain Unicode punctuation
    (em-dashes, smart quotes, bullets, etc.) somewhere below the SDK
    surface. Anything sent to the model — including startup names/
    descriptions/logs that get echoed back into later prompts — is
    sanitized to pure ASCII here as a single choke point."""
...
def call_llm(api_key: str, system_prompt: str, user_prompt: str, fallback: str) -> str:
    """Calls Gemini, trying the last known-good model first, then walking
    the candidate list on a NOT_FOUND/retirement error. Any other failure
    (or no key configured) returns a deterministic mock so the app always
    works end-to-end."""
    if not api_key or not GENAI_AVAILABLE:
        return fallback

    override = (st.session_state.get("model_override") or "").strip()
    working = st.session_state.get("_working_model")
    ordered = [m for m in [override, working] if m] + [
        m for m in MODEL_CANDIDATES if m not in (override, working)
    ]

    messages = [
        SystemMessage(content=to_ascii(system_prompt)),
        HumanMessage(content=to_ascii(user_prompt)),
    ]
    ...
    for model_name in ordered:
        ...
        try:
            ...
            st.session_state._working_model = model_name  # remember what worked
            return text if text else fallback
        except Exception as e:
            last_err = e
            continue  # this model name is stale/unavailable — try the next one

    return f"{fallback}\n\n_(mock fallback - live model unavailable: {str(last_err)[:140]})_"
...
def global_coo_node(state: GraphState) -> dict:
    ...
    raw = call_llm(api_key, system, user, fallback)
...
def local_pm_node(state: GraphState) -> dict:
    ...
    plan = call_llm(api_key, system, user, fallback)
...
def researcher_node(state: GraphState) -> dict:
    ...
    research = call_llm(api_key, system, user, fallback)
...
def engineer_node(state: GraphState) -> dict:
    ...
    code = call_llm(api_key, system, user, fallback)
...
def pm_review_node(state: GraphState) -> dict:
    ...
    review = call_llm(api_key, system, user, fallback)
...
def init_session() -> None:
    ...
    if "_working_model" not in st.session_state:
        st.session_state._working_model = None
...
with st.sidebar:
    ...
    if key_input != st.session_state.api_key:
        ...
        st.session_state._working_model = None  # re-probe candidates with the new key

    with st.expander("⚙️ Model settings", expanded=False):
        st.caption(
            "Google renames/retires Flash model IDs often. The app tries "
            "`gemini-flash-latest` first, then falls through a few known "
            "generations automatically. Only set this if you need to force "
            "a specific model."
        )
        override_input = st.text_input(
            "Force model name (optional)",
            value=st.session_state.model_override,
            placeholder="e.g. gemini-3.5-flash",
        )
        if override_input != st.session_state.model_override:
            st.session_state.model_override = override_input
            st.session_state._working_model = None

    if not st.session_state.api_key or not GENAI_AVAILABLE:
        status = "🟡 Demo mode — mock agents (no API key set)"
    elif st.session_state._working_model:
        status = f"🟢 Live — `{st.session_state._working_model}`"
    else:
        status = "🟡 Demo mode — will re-probe live models on next message"
    st.caption(status)
    st.divider()
...
st.caption("Global COO orchestrating autonomous startup families via LangGraph + Gemini Flash (auto-selected)")
